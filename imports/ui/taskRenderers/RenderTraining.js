...
const RenderTraining = ({ task, updateTask }) => {
    ...
    const onGetNextQuest = useCallback(async ({ current_quest, current_content, done_quests, done_contents }) => {
    ...
    }, [task, outroContent, doneContents])
    ...
    }, [task, content, currentQuestId, doneQuests])
    ...
    return <>
        <RenderQuest task={task} question={question} isLastQuestion={isLastQuestion} onGetNextQuest={onGetNextQuest} />
    </>
}
